...
TEST_CASE("WHEN_given_defaul_value_is_invalid_THEN_returns_EINVAL", component)
{
	...
	uint8_t invalid = 10;
	err = mcp23017_enable_pin_intrrupt(dev, 1, invalid, ON_CHANGE_FROM_REG);
...
TEST_CASE("WHEN_given_pin_is_invalid_THEN_returns_EINVAL", component)
{
	...
	/* when pin is invalid */
	uint8_t invalid = 16;
	err = mcp23017_enable_pin_intrrupt(dev, invalid, 1, ON_CHANGE_FROM_REG);
...
TEST_CASE("WHEN_set_pin_8_is_set_THEN_returns_zero", component)
{
	...
	err = mcp23017_enable_pin_intrrupt(dev, 8, HIGH, ON_CHANGE_FROM_REG);
