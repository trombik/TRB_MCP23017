...
TEST_CASE("WHEN_given_defaul_value_is_invalid_THEN_returns_EINVAL", component)
{
	...
	uint8_t invalid = 10;
	err = mcp23017_enable_pin_intrrupt(1, invalid, ON_CHANGE_FROM_REG);
...
TEST_CASE("WHEN_given_pin_is_invalid_THEN_returns_EINVAL", component)
{
	my_setup();

	/* when pin is invalid */
	uint8_t invalid = 16;
	err = mcp23017_enable_pin_intrrupt(invalid, 1, ON_CHANGE_FROM_REG);
...
TEST_CASE("WHEN_set_pin_8_is_set_THEN_returns_zero", component)
{
	...
	err = mcp23017_enable_pin_intrrupt(8, HIGH, ON_CHANGE_FROM_REG);

	/* then writes 1 to three registers, returns zero */
	TEST_ASSERT_EQUAL_INT32(0, err);
	TEST_ASSERT_EQUAL_UINT8(MCP23x17_DEFVALB, mcp23017_write8_fake.arg0_history[0]);
	TEST_ASSERT_EQUAL_UINT8(expected, mcp23017_write8_fake.arg1_history[0]);
	TEST_ASSERT_EQUAL_UINT8(MCP23x17_INTCONB, mcp23017_write8_fake.arg0_history[1]);
	TEST_ASSERT_EQUAL_UINT8(expected, mcp23017_write8_fake.arg1_history[1]);
	TEST_ASSERT_EQUAL_UINT8(MCP23x17_GPINTENB, mcp23017_write8_fake.arg0_history[2]);
	TEST_ASSERT_EQUAL_UINT8(expected, mcp23017_write8_fake.arg1_history[2]);
